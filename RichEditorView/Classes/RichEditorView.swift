...
extension RichEditorView {
    ...
    public func setHTML(_ html: String) {
        contentHTML = html
        if editorLoaded {
            let _ = runJS("RE.setHtml('\(escape(html))');")
            updateHeight()
        }
    }
    ...
    public func setPlaceholderText(_ text: String) {
        placeholder = text
        let _ = runJS("RE.setPlaceholderText('\(escape(text))');")
    }
    
    public func removeFormat() {
        let _ = runJS("RE.removeFormat();")
    }
    
    public func setFontSize(_ size: Int) {
        let _ = runJS("RE.setFontSize('\(size)px');")
    }
    
    public func setEditorBackgroundColor(_ color: UIColor) {
        let hex = colorToHex(color)
        let _ = runJS("RE.setBackgroundColor('\(hex)');")
    }
    ...
    public func bold() {
        let _ = runJS("RE.setBold();")
    }
    
    public func italic() {
        let _ = runJS("RE.setItalic();")
    }
    
    // "superscript" is a keyword
    public func subscriptText() {
        let _ = runJS("RE.setSubscript();")
    }
    
    public func superscript() {
        let _ = runJS("RE.setSuperscript();")
    }
    
    public func strikethrough() {
        let _ = runJS("RE.setStrikeThrough();")
    }
    
    public func underline() {
        let _ = runJS("RE.setUnderline();")
    }
    
    public func setTextColor(_ color: UIColor) {
        let _ = runJS("RE.prepareInsert();")
        
        let hex = colorToHex(color)
        let _ = runJS("RE.setTextColor('\(hex)');")
    }
    ...
    public func setTextBackgroundColor(_ color: UIColor) {
        ...
        let hex = colorToHex(color)
        let _ = runJS("RE.setTextBackgroundColor('\(hex)');")
    }
    
    public func header(_ h: Int) {
        let _ = runJS("RE.setHeading('\(h)');")
    }

    public func indent() {
        let _ = runJS("RE.setIndent();")
    }

    public func outdent() {
        let _ = runJS("RE.setOutdent();")
    }

    public func orderedList() {
        let _ = runJS("RE.setOrderedList();")
    }

    public func unorderedList() {
        let _ = runJS("RE.setUnorderedList();")
    }

    public func blockquote() {
        let _ = runJS("RE.setBlockquote()");
    }
    
    public func alignLeft() {
        let _ = runJS("RE.setJustifyLeft();")
    }
    
    public func alignCenter() {
        let _ = runJS("RE.setJustifyCenter();")
    }
    
    public func alignRight() {
        let _ = runJS("RE.setJustifyRight();")
    }
    
    public func insertImage(_ url: String, alt: String) {
        let _ = runJS("RE.prepareInsert();")
        let _ = runJS("RE.insertImage('\(escape(url))', '\(escape(alt))');")
    }
    
    public func insertLink(_ href: String, title: String) {
        let _ = runJS("RE.prepareInsert();")
        let _ = runJS("RE.insertLink('\(escape(href))', '\(escape(title))');")
    }
    
    public func focus() {
        let _ = runJS("RE.focus();")
    }
    
    public func blur() {
        let _ = runJS("RE.blurFocus()")
    }
...
extension RichEditorView {
    ...
    fileprivate func setContentEditable(_ editable: Bool) {
        ...
        if editorLoaded {
            ...
            let _ = runJS("RE.editor.contentEditable = \(value);")
        }
    }
...
//        let unicode = Array(string.unicodeScalars)
        var newString = ""
    ...
    */
    fileprivate func performCommand(_ method: String) {
        if method.hasPrefix("ready") {
            ...
            let range = Range(actionPrefix.startIndex..<actionPrefix.endIndex)
